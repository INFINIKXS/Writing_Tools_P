---
archived: 2026-08-07T20:02:30.227395
source: C:\Users\Paradox-Labs\.gemini\antigravity\brain\60a5c1f3-7d3a-4ec0-9142-9adc7d41fae0\walkthrough.md
---

# Step 1, 2, 3, 4, 5 & 6 Implementation Walkthrough: Font Metric Corruption, Word Placement, X-Height Calibration & Emit-Font-Aligned Detection

## Overview

This walkthrough documents the complete 6-step solution to baked-font metric corruption, oversized text rendering, spacing alignment, true outline-measured x-height calibration, and emit-font-aligned glyph detection in PDF edits.
...
1. **Probe Emission Font Bytes**:
   ...
   - Added `logger.info(f"[BAD] {len(missing_chars)} unrenderable chars against emit font {font_name_actual!r}")`.
2. **Per-Run Floor in Emitters**:
   - `sub_chars = [ch for ch in text if ch in resolver or ch in still or ch in hard_chars]` (with `hard_chars=bad`).
...
- **Coverage Detection Alignment**: `emit_buf` probing guarantees 0 unrenderable character drops during bake.
